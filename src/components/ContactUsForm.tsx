...
export function ContactUsForm({ id, country }: ContactUsFormProps) {
    ...
    return (
      <div id={`${id}-success`} className="p-6 md:p-8 w-full max-w-xl mx-auto" data-success={isSuccess}>
        <div className="text-center">
          ...
          <h3 className="mt-4 font-display text-xl font-bold text-ink">Your Enquiry has been sent!</h3>
          <p className="mt-1 text-sm text-muted">We'll review your enquiry and get back to you asap</p>
        </div>
  ...
  return (
    <div className="p-6 md:p-8" data-success={isSuccess}>
      <h4 className="font-display text-lg font-semibold text-ink">Enquiry Form</h4>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-3" noValidate>
        ...
        <div>
          <label htmlFor={`${id}-business-name`} className="block text-xs font-semibold text-ink">
            Business Name
          </label>
          <Input
              id={`${id}-business-name`}
          ...
          />
          {errors.businessName && (
              <p className="mt-0.5 text-xs text-accent-coral">{errors.businessName.message}</p>
          )}
        </div>

        <div>
          <label htmlFor={`${id}-message`} className="block text-xs font-semibold text-ink">
            Message
          </label>
          <Textarea
              id={`${id}-message`}
          ...
          />
          {errors.message && (
              <p className="mt-0.5 text-xs text-accent-coral">{errors.message.message}</p>
          )}
        </div>
