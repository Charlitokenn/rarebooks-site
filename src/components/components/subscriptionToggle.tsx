                    ...
                    plans.map((plan) => {
                        ...
                        return (
                            ...
                            >
                                {plan.featured && (
                                    <span
                                        className="mb-4 inline-flex w-fit rounded-pill bg-brand px-3 py-1 text-xs font-semibold text-white">
                Most popular
              </span>
                                )}
                                <h3 className="font-display text-xl font-bold">{plan.name}</h3>
                                <div className="mt-4 flex items-end gap-1">
              <span className={cn("font-display font-extrabold tracking-tight", isLocal ? "text-4xl" : "text-5xl")}>
                {isCustom ? price : monthly ? symbol + (isLocal ? price : price[1]) : symbol + (isLocal ? price : price[0])}
              </span>
                                    {!isCustom && (
                                        <span className={plan.featured ? "mb-1.5 text-white/60" : "mb-1.5 text-muted"}>
                  {isLocal ? !monthly ? "/Year" : "/Month" : !monthly ? "/Month" : "/Year"}
                </span>
                                    )}
                                </div>
                                <p className={`mt-3 text-sm leading-relaxed ${plan.featured ? "text-white/70" : "text-muted"}`}>
                                    {plan.blurb}
                                </p>
